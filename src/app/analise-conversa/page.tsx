...
export default function AnaliseConversaPage() {
    ...
    const handleAnalyzeClick = async () => {
        if (!image) return;
        trackEvent('click_analyze_conversation');
        setLoading(true);
            ...
            <MainLayout activeTab="grupos">
                <main className="p-4 sm:p-6 md:p-8">
                    <div className="max-w-2xl mx-auto">
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle as="h1" className="text-3xl font-bold">Analista de Conversa com IA</CardTitle>
                                <CardDescription>
                                    Empacou no papo? Envie um print da sua conversa e deixe nossa IA sugerir sua próxima mensagem.
                                </CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                <div 
                                    className="w-full aspect-video border-2 border-dashed border-muted-foreground rounded-lg flex items-center justify-center bg-muted/20 cursor-pointer hover:bg-muted/40 transition-colors"
                                    onClick={triggerFileSelect}
                                >
                                    {image ? (
                                        <Image src={image} alt="Print da conversa" width={300} height={600} className="object-contain rounded-lg max-h-full w-auto" />
                                    ) : (
                                        <div className="text-center text-muted-foreground">
                                            <Upload className="mx-auto h-12 w-12" />
                                            <p className="mt-2">Clique para enviar um print</p>
