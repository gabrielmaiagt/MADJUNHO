...
import type { TransactionData, Product, CustomerInfo, CheckoutInfo } from '@/lib/types';
...
export default function ProductDetailPage() {
    ...
    const handleBuyNow = async () => {
        ...
        const source = `product_page:${product.id}`;
        trackDetailedEvent('click_buy_now', { price: product.price, productName: product.name, source });
        
        setIsLoading(true);

        try {
            const utm_params = JSON.parse(localStorage.getItem('utm_params') || '{}');
            const visitorId = localStorage.getItem('utmify_visitor_id');
            ...
            const newTransaction = await createStoreTransaction({
                ...
                tracking: {
                    utm_params,
                    visitorId,
                    xcod: utm_params.xcod,
                }
            });
            setTransaction(newTransaction);
            trackDetailedEvent('generate_pix', { amount: product.price, productName: product.name, source: source });
                        ...
                        <div className="flex items-center justify-center gap-3">
                            ...
                            >
                             ...
                             <Button
                                ...
                                disabled={isLoading}
                             >
                               {isLoading ? <Loader2 className="animate-spin" /> : 'Comprar Agora'}
                            </Button>
                        </div>
                    </footer>
                </div>
